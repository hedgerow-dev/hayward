...
| `MFV-ARCHIVE-002` | MEDIUM | 22, 59 | An archive member is a symlink or hard link whose target escapes the extraction directory (a `..` segment, an absolute path, a Windows drive path, a UNC path, or an embedded NUL/newline). Distinct from MFV-ARCHIVE-001: here the member *is* the link. On extraction the loader creates the link, then a later member written through it lands outside the target directory (the classic tar/zip symlink attack). Covers the tar-based `.nemo` (tar sym/hard links) and the zip containers `.pt`/`.mar` (a member with the `S_IFLNK` unix mode in its external attributes, its body the link target); a link to a safe in-tree target such as `weights/shard1` does not fire. MEDIUM matches MFV-ARCHIVE-001: Hayward never extracts, so the sink is a downstream unpacker |
